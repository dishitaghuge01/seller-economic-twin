...
const DICT = {
  ...
  "login.phoneLabel": { hi: "फोन नंबर", en: "Phone number" },
  ...
  "forecast.restockUrgent": { hi: "स्टॉक तुरंत भरें।", en: "Restock urgent." },
  ...
  "wa.composePlaceholder": { hi: "प्राइसिंग के बारे में पूछिए…", en: "Ask about pricing…" },
...
const fallbackT = (key, vars) => {
  ...
  if (!entry) return key;
  const v = entry.en ?? key;
  if (typeof v === "function") return v(vars || {});
  return v;
};

const LangCtx = createContext({ lang: "en", setLang: (_next) => { void _next; }, t: fallbackT });
...
export function LanguageProvider({ children }) {
  ...
  const t = useCallback((key, vars) => {
    const entry = DICT[key];
    if (!entry) return key;
    const v = entry[lang] ?? entry.en ?? key;
    if (typeof v === "function") return v(vars || {});
    return v;
  }, [lang]);
